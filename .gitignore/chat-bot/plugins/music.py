...
class Music(Plugin):
    ...
    async def play(self, m, args):
        ...
        if not voice:
            ...
            return await self.multigaming.send_message(m.channel, response)
        ...
        try:
            ...
            print(e)
            await self.multigaming.send_message(m.channel, response)
    ...
    async def stop(self, m, args):
        await self.multigaming.schwifty.stop(m.server.id)
    ...
    async def join(self, message, args):
        ...
        if not voice_channel:
            ...
            return await self.multigaming.send_message(message.channel, response)

        await self.multigaming.schwifty.voice_connect(message.server.id)
        await self.multigaming.ws.voice_state(message.server.id, voice_channel.id)
    ...
    async def playlist(self, message, args):
        ...
        fmt = "\n `Full playlist ` <https://multigaming.xyz/request_playlist/{}>"
    ...
    async def add(self, message, args):
        ...
        if 'http' in search:
            ...
            try:
                ...
                return await self.multigaming.send_message(message.channel,
                                                    response)
    ...
    async def get_audio_info(self, url):
        ...
        info = await self.multigaming.loop.run_in_executor(None, func)
        if "entries" in info:
            info = info['entries'][0]
