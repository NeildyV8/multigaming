...
multigaming_COLOR = int('008cba', 16)

def check_add_role_perm(member, role, multigaming):
    permissions = multigaming.server_permissions
    return permissions.manage_roles and multigaming.top_role > role
...
class Levels(Plugin):
    ...
    async def levels(self, message, args):
        url = "<http://multigaming.xyz/levels/" + message.server.id + ">"
    ...
    async def rank(self, message, args):
        ...
        if not player_info:
            ...
            if player != message.author:
                resp = "{}, It seems like " + player.mention + \
                    " is not ranked :cry:."
            await self.multigaming.send_message(message.channel,
        ...
        if message.channel.permissions_for(message.server.me).embed_links:
            ...
            embed.set_footer(text='multigaming.xyz', icon_url=MEE6_ICON)

            return await self.multigaming.send_message(message.channel, embed=embed)
    ...
    async def on_message(self, message):
        if message.author.id == self.multigaming.user.id or message.author.bot:
            return
        ...
        await self.multigaming.db.redis.set('server:{}:name'.format(server.id),
                                     server.name)
        if server.icon:
            await self.multigaming.db.redis.set('server:{}:icon'.format(server.id),
                                         server.icon)
        if server.icon:
        ...
        if new_level != lvl:
            ...
            if announcement_enabled:
                ...
                try:
                    await self.multigaming.send_message(dest, announcement.replace(
                        "{player}",
                        mention,
                    ).replace(
                        "{level}",
