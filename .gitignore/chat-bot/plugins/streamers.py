...
class Streamers(Plugin):
    ...
    async def get_servers_list(self):
        servers = []
        for server in list(self.multigaming.servers):
            plugins = await self.multigaming.db.redis.smembers("plugins:"+server.id)
    ...
    async def get_live_streamers_by_servers(self):
        ...
        for platform in self.platforms:
            ...
            for server in servers:
                server_streamers = await self.multigaming.db.redis.smembers(
    ...
    async def streamer_check(self):
        ...
        for server_id, live_streamers in data.items():
            server = self.multigaming.get_server(server_id)
            if not server:
                continue

            storage = await self.get_storage(server)
            channel_id = await storage.get('announcement_channel')
            announcement_channel = self.multigaming.get_channel(channel_id) or server
