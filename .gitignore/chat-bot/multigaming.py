...
class multigaming(discord.Client):
    ...
    async def add_all_servers(self):
        ...
        for server in self.servers:
            self.stats.set('multigaming.servers', server.id)
    ...
    async def on_server_join(self, server):
        ...
        self.stats.set('multigaming.servers', server.id)
        self.stats.incr('multigaming.server_join')
        await self.db.redis.sadd('servers', server.id)
    ...
    async def send_message(self, *args, **kwargs):
        self.stats.incr('multigaming.sent_messages')
        return await super().send_message(*args, **kwargs)

    async def on_message(self, message):
        self.stats.incr('multigaming.recv_messages')
